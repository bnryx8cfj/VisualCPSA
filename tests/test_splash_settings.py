...
class VisualCPSASplashTests(unittest.TestCase):
    ...
    def test_exchange_exports_send_recv(self) -> None:
        ...
        diagram.lifelines.extend([LifelineView(participant_id=alice.id, x=100.0), LifelineView(participant_id=bob.id, x=400.0)])
        ...
        diagram.message_views.append(MessageExchangeView(exchange_id=exchange.id, source_lifeline_id=diagram.lifelines[0].id, target_lifeline_id=diagram.lifelines[1].id, y=130.0))
